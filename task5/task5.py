...
а потім, за вашим вибором, {Fore.LIGHTGREEN_EX}СТВОРЮЄМО ДЕРЕВО З КУПИ! \
{Fore.LIGHTBLUE_EX}Запускаємо алгоритм BFS! \
{Fore.LIGHTMAGENTA_EX}Запускаємо алгоритм DFS!{Style.RESET_ALL} ")
    ...
    if user_input not in heap_composition:
        ...
        print(f"\n{Fore.LIGHTGREEN_EX}Початкова купа {Style.RESET_ALL}")
        draw_heap(root_heap)

        # BFS та зміна кольору
        bfs_order = bfs_algorithm(root_heap)
        make_colors(bfs_order)
        print(f"\n{Fore.LIGHTGREEN_EX}Обхід в ШИРИНУ {Style.RESET_ALL}")
        draw_heap(root_heap)

        # DFS та зміна кольорів
        dfs_order = dfs_algorithm(root_heap)
        make_colors(dfs_order)
        print(f"\n{Fore.LIGHTGREEN_EX}Обхід у ГЛИБИНУ{Style.RESET_ALL}")
        draw_heap(root_heap)

        print(
            f"\n{Fore.MAGENTA}Дякую, що подивились виконання роботи!{Style.RESET_ALL}")
